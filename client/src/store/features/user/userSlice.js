...
export const actionChangeOnlineStatus = createAsyncThunk(
   'user/actionChangeOnlineStatus',
   async (formData, thunkApi) => {
      try {
         return await serviceChangeStatus(formData);
      } catch (error) {
         const message =
            (error.response &&
               error.response.data &&
...
const userSlice = createSlice({
   ...
   extraReducers: (builder) => {
      builder
         ...
         .addCase(actionChangeOnlineStatus.pending, (state) => {
            state.isLoading = true;
         })
         .addCase(actionChangeOnlineStatus.fulfilled, (state, action) => {
            state.isLoading = false;
            state.isSuccess = true;
            state.user = action.payload;
         })
         .addCase(actionChangeOnlineStatus.rejected, (state, action) => {
            state.isLoading = false;
            state.isError = true;
            state.message = action.payload;
         });
...
export const { resetUser, updateUserDetail } = userSlice.actions;
